...
// ── Continuous single-hue gradient strings for the legend ────────────────────
// These match the matplotlib colormaps used in the API renderer.
// Low score (left) = light end; High score (right) = saturated/dark end.
const CMAP_GRADIENTS = {
  YlGn:    'linear-gradient(to right, #ffffe5, #d9f0a3, #addd8e, #78c679, #41ab5d, #238443, #005a32)',
  Blues:   'linear-gradient(to right, #f7fbff, #c6dbef, #9ecae1, #6baed6, #4292c6, #2171b5, #084594)',
  OrRd:    'linear-gradient(to right, #fff7ec, #fdd49e, #fdbb84, #fc8d59, #ef6548, #d7301f, #7f0000)',
  YlOrBr:  'linear-gradient(to right, #ffffe5, #fff7bc, #fee391, #fec44f, #fe9929, #cc4c02, #662506)',
  Greys:   'linear-gradient(to right, #ffffff, #d9d9d9, #bdbdbd, #969696, #737373, #525252, #252525)',
};

const LAYER_META = {
  suitability: { label: 'Suitability',  unit: '/100',   low: 'Low',    high: 'High',  gradient: null },
  elevation:   { label: 'Elevation',    unit: 'm',      low: 'Low',    high: 'High',  gradient: CMAP_GRADIENTS.YlGn },
  rainfall:    { label: 'Rainfall',     unit: 'mm/yr',  low: 'Dry',    high: 'Wet',   gradient: CMAP_GRADIENTS.Blues },
  temperature: { label: 'Temperature',  unit: '°C',     low: 'Cool',   high: 'Hot',   gradient: CMAP_GRADIENTS.OrRd },
  soil:        { label: 'Soil Clay',    unit: 'g/kg',   low: 'Sandy',  high: 'Clay',  gradient: CMAP_GRADIENTS.YlOrBr },
  slope:       { label: 'Slope',        unit: '°',      low: 'Flat',   high: 'Steep', gradient: CMAP_GRADIENTS.Greys },
};
...
function Legend({ activeLayer, result, countyInfo, boundaryLoaded }) {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(255,255,255,0.97)', border: '1px solid #c8d8b8',
      padding: '0.65rem 0.85rem', borderRadius: '8px', minWidth: '170px',
      ...
      {activeLayer === 'suitability' ? (
        <>
          {[
            ...
            { color: '#e8ede0', label: 'Excluded',          note: 'Protected',
          ...
          {result && (
            <div style={{
              ...
              borderTop: '1px solid #dde5d4', fontSize: '0.7rem', color: '#5a7a42',
            }}>
        ...
        <>
          ...
          <div style={{
            ...
            fontSize: '0.62rem', color: '#7a8f68', marginBottom: '3px',
          }}>
            <span>{meta.low}</span>
            <span>{meta.high}</span>
          </div>
      ...
      {boundaryLoaded && (
        <div style={{
          display: 'flex', alignItems: 'center', gap: '0.45rem',
          fontSize: '0.7rem', marginTop: '0.4rem', paddingTop: '0.35rem',
...
function MapView({ analysisResult, countyInfo, apiBaseUrl, activeCounty, activeLayer }) {
  ...
  // Fetch boundary GeoJSON whenever county changes
  useEffect(() => {
  ...
  // Update overlay when active layer or analysis changes
  useEffect(() => {
    ...
    if (activeLayer === 'suitability') {
      ...
      setOverlayUrl(
        `${apiBaseUrl}/map-image/${analysisResult.analysis_id}`
        + `?t=${Date.now()}&county=${activeCounty}`
      );
      setOverlayBounds(analysisResult.raster_bounds);
      return;
    }

    // Factor layer — requires a completed analysis for bounds
    ...
    img.src = url;
  }, [activeLayer, activeCounty, analysisResult?.analysis_id]); // eslint-disable-line
    ...
    <div className="map-container">
      ...
      {layerLoading && (
        <div style={{
          ...
          background: 'rgba(255,255,255,0.95)', border: '1px solid #c8d8b8',
          borderRadius: '6px', padding: '0.3rem 0.8rem',
          fontSize: '0.72rem', color: '#5a7a42', zIndex: 1000,
        }}>
      ...
      {activeLayer !== 'suitability' && !analysisResult?.raster_bounds && !layerLoading && (
        <div style={{
          ...
          Run an analysis first to view factor layers
        </div>
      )}
